...
We trained and fine-tuned **Gemma 4 E4B** on an **NVIDIA RTX PRO 6000 Blackwell GPU (96 GB VRAM)** using **Unsloth QLoRA** (r=16) — loss **1.79 → 0.57** over 380 steps. The training set is **3,875 instruction pairs** (plus a 430-example held-out eval split), generated by [`gemma_model/scripts/1_generate_data.py`](gemma_model/scripts/) and mixing three sources:

1. **Programmatic pairs** generated from the app's real tool schemas and content packs — M-Pesa tariff tables, matatu fares, UoEm fee/HELB documents — so tool calls and document extraction train on known-correct outputs.
2. **Teacher distillation** — Gemma 4 26B/31B generating trilingual (English/Kiswahili/Sheng) campus-scenario conversations.
3. **Public Swahili instruction data** (the Aya collection) plus our **hand-collected Kiembu phrasebook** — Embu's actual local language has essentially zero public NLP data, so we collected our own with students.
...
| On-device `.litertlm` build (int4, 4.26 GB) | [`Eugeniuss/gemma-4-tcc-e4b-litertlm`](https://huggingface.co/Eugeniuss/gemma-4-tcc-e4b-litertlm) — built with our [conversion pipeline](gemma_model/) (`litert-torch export_hf`, int4) |
